...
class TestGetCommentReplies(unittest.TestCase):
    ...
    def test_comment_replies(self):
        ...
        reply_dto_1 = CommentDto(1, 1, datetime.now(), "comment1", commented_on_id=1)
        reply_dto_2 = CommentDto(2, 1, datetime.now(), "comment2", commented_on_id=2)
        reply_dto_3 = CommentDto(3, 1, datetime.now(), "comment3", commented_on_id=3)
        comment_with_person_dto_list = [CommentWithPersonDto(reply_dto_1, person_dto),
                                        CommentWithPersonDto(reply_dto_2, person_dto),
                                        CommentWithPersonDto(reply_dto_3, person_dto)]
        json_presenter = Presenter()
        response = json_presenter.get_comment_replies_response(comment_with_person_dto_list)
        ...
        assert reply_response['commenter']['name'] == person_dto.username
        assert reply_response['commenter']['user_id'] == person_dto.id
        assert reply_response['commenter']['profile_pic_url'] == person_dto.profile_url_pic
