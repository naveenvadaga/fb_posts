...
class TestGetPostDetails(unittest.TestCase):
    ...
    def post_details_set_up(self):
        person_dto_1 = PersonDto(1, "username1", "https://www.profile_url_pic.com")
        person_dto_2 = PersonDto(2, "username2", "https://www.profile_url_pic.com")
        person_dto_3 = PersonDto(3, "username3", "https://www.profile_url_pic.com")
        ...
        self.user_post_dto = UserPostDto(post_dto, person_dto_list, reaction_dto_list, comment_dto_list)

    def get_post_utility_functions_setup(self):
        ...
        self.comment_dict = self.get_comments_dict(self.user_post_dto.comments, self.persons_dict,
                                                   self.comment_wise_reactions_stats_dict)
    ...
    def comment_dict_insert_record(self, reaction):
        self.comment_wise_reactions_stats_dict[reaction.comment_id]["count"] = \
            self.comment_wise_reactions_stats_dict[
                reaction.comment_id][
                "count"] + 1
        self.comment_wise_reactions_stats_dict[reaction.comment_id]["type"].append(reaction.reaction_type)
    ...
    def test_post_details(self):
        ...
        for person in self.user_post_dto.persons:
            if person.id == response['posted_by']['user_id']:
                person_dto = person
        assert response['post_id'] == post_dto.id
        assert response['posted_at'] == post_dto.posted_at.strftime("%Y-%m-%d, %H:%M:%S")
        assert response['post_content'] == post_dto.post_content
        assert response['posted_by']['user_id'] == person_dto.id
        assert response['posted_by']['name'] == person_dto.username
        assert response['posted_by']['profile_pic_url'] == person_dto.profile_url_pic
    ...
    def test_comment_stats(self):
        ...
        for comment in self.user_post_dto.comments:
            if comment.comment_id == response_comment['comment_id']:
                comment_dto = comment
        for person in self.user_post_dto.persons:
            if person.id == response_comment['commenter']['user_id']:
                person_dto = person
        assert response_comment['comment_id'] == comment_dto.comment_id
        assert response_comment['commented_at'] == comment_dto.commented_at.strftime("%Y-%m-%d, %H:%M:%S")
        assert response_comment['comment_content'] == comment_dto.comment_content
        assert response_comment['commenter']['user_id'] == person_dto.id
        assert response_comment['commenter']['name'] == person_dto.username
        assert response_comment['commenter']['profile_pic_url'] == person_dto.profile_url_pic
    ...
    def test_reaction_stats(self):
        ...
        assert response_comment['reactions']['count'] == len(comment_reaction_types)

    def test_person_dict(self):
        self.get_post_utility_functions_setup()
        person_dto_list = self.user_post_dto.persons
        json_presenter = Presenter()
        response = json_presenter.get_persons_dict(person_dto_list)
    ...
    def test_comment_wise_reaction_stats_dict(self):
        ...
        reaction_dto_list = self.user_post_dto.reactions
        json_presenter = Presenter()
        comment_wise_dict = json_presenter.get_comment_wise_reaction_stats_dict(reaction_dto_list)
    ...
    def test_get_comment_dict(self):
        ...
        json_presenter = Presenter()
        response = json_presenter.get_comment_dict(comment_dto, person_dto_list, reaction_dto_list)
        assert response['comment_id'] == comment_dto.comment_id
        assert response['commented_at'] == comment_dto.commented_at.strftime("%Y-%m-%d, %H:%M:%S")
    ...
    def test_get_comments_details_dict(self):
        ...
        comment_dto_list = self.user_post_dto.comments
        json_presenter = Presenter()
        response = json_presenter.get_comment_details_dict(comment_dto_list, self.comment_dict)
        comment = response[0]
        setup_comment = self.comment_dict[comment['comment_id']]
        assert len(response) == 1

        assert comment['comment_id'] == setup_comment['comment_id']
        assert comment['commenter']['user_id'] == setup_comment['commenter']['user_id']
