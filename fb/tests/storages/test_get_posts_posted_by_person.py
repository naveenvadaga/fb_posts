...
class TestPostsPostedByPersonDetails:
    ...
    def post_details_setup(self, post_setup):
        ...
        self.reply2_for_comment1 = Comment.objects.create(
            ...
            reply_id=self.comment1_for_post.id)
        self.reply2_commented_person = self.person3

    def test_get_posts_posted_by_person_post_dto(self, post_details_setup):
        post_id = self.post.id
        storage = StorageClass()
        post_details_list = storage.get_posts_posted_by_person(
    ...
    def test_get_posts_posted_by_person_comment_dto(self, post_details_setup):
        storage = StorageClass()
        post_details_list = storage.get_posts_posted_by_person(
            self.posted_person.id, 0, 1)
        post_details = post_details_list[0]
    ...
    def test_get_posts_posted_by_person_reply_dto(self, post_details_setup):
        storage = StorageClass()
        post_details_list = storage.get_posts_posted_by_person(
            self.posted_person.id, 0, 1)
        post_details = post_details_list[0]
