...
class TestPostDetails:
    ...
    def comment_details_setup(self, post_details_setup):
        ...
        self.comment_dict = {
            self.comment1_for_post.id: self.comment1_for_post,
            self.reply1_for_comment1.id: self.reply1_for_comment1,
            self.reply2_for_comment1.id: self.reply2_for_comment1}

    def test_get_post_details_post_dto(self, comment_details_setup):
        post_id = self.post.id
    ...
    def test_get_post_details_persons_dto(self, comment_details_setup):
        post_id = self.post.id
        storage = StorageImplementer()
        post_details = storage.get_post_details(post_id)
    ...
    def test_get_post_details_comments_dto(self, comment_details_setup):
        post_id = self.post.id
        storage = StorageImplementer()
        post_details = storage.get_post_details(post_id)
