...
class TestGetPositiveReactedPosts(unittest.TestCase):
    ...
    def test_get_positive_reacted_posts_returns_post_ids_list(self):
        ...
        mock_json_presenter.get_positive_reacted_posts_response.return_value = mock_response

        post_interactor = PostInteractor(mock_json_presenter, mock_storage)
        response_from_get_post_metrics = post_interactor.get_more_positive_reacted_post_ids_interactor()

        mock_storage.get_more_positive_reacted_post_ids.assert_called_once()
        mock_json_presenter.get_positive_reacted_posts_response.assert_called_once_with(
            posts_id)
