...
class TestAddCommentToPost(unittest.TestCase):
    def test_add_comment_to_post_returns_comment_id(self):
        ...
        mock_storage.add_comment_to_post_interactor.return_value = comment_id
        ...
        comment_interactor = CommentInteractor(mock_json_presenter,
                                               mock_storage)
        response_from_get_reactions_count_post = comment_interactor.add_comment_to_post_interactor(
            post_id, commenter_id,
            comment_text)

        mock_storage.add_comment_to_post_interactor.assert_called_once_with(post_id,
                                                                            commenter_id,
                                                                            comment_text)
