...
class TestReactToCommment(unittest.TestCase):
    ...
    def test_react_to_comment_returns_reactions_id(self):
        ...
        mock_storage.react_to_comment_exists.side_effect = ObjectDoesNotExist
        ...
        response_from_react_to_comment = react_interactor.react_to_comment(reacted_by_id, comment_id, reaction_type)

        mock_storage.react_to_comment_exists.assert_called_once_with(reacted_by_id, comment_id)
        mock_storage.react_to_comment.assert_called_once_with(reacted_by_id, comment_id, reaction_type)
        assert response_from_react_to_comment == response
    ...
    def test_react_to_comment_when_reactions_by_same_person_with_same_reaction_type(self):
        ...
        mock_storage.react_to_comment_exists.return_value = mock_react_dto
    ...
    def test_react_to_comment_when_reactions_by_same_person_with_different_reaction_type(self):
        mock_storage = Mock(spec=Storage)
        mock_json_presenter = Mock(spec=JsonPresenter)
        reacted_by_id = 1
        comment_id = 1
