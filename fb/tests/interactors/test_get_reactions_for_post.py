...
class TestGetReactionsForPost(unittest.TestCase):
    ...
    def test_get_reactions_for_post_returns_reactions_for_post(self):
        ...
        mock_reaction1 = Mock(spec=[field.name for field in fields(PersonWithReactionDto)])
        mock_reaction2 = Mock([field.name for field in fields(PersonWithReactionDto)])
        ...
        response_from_get_reactions_count_post = post_interactor.get_post_reactions_interactor(
            post_id, offset, limit)
        mock_storage.get_post_reactions.assert_called_once_with(post_id, offset, limit)
        mock_json_presenter.create_post_reactions_response.assert_called_once_with(
            mock_reaction_list)
