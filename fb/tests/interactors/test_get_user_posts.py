...
class TestGetPostsPostedByPerson(unittest.TestCase):
    ...
    def test_get_user_posts_returns_list_of_posts_details(self):
        ...
        person_id = 1
        mock_get_post1_dto = Mock(spec=[field.name for field in fields(UserPostDto)])
        mock_get_post2_dto = Mock(spec=[field.name for field in fields(UserPostDto)])
        ...
        response_from_get_posts_posted_by_person = post_interactor.get_user_posts_interactor(
            person_id)
