...
class TestGetPostReactedByUser(unittest.TestCase):
    ...
    def test_get_user_reacted_posts_returns_post_ids(self):
        ...
        response_from_get_post_count_method = post_interactor.get_user_reacted_posts_interactor(
            user_id)
        mock_storage.get_post_reacted_by_user.assert_called_once_with(user_id)
        mock_json_presenter.get_user_reacted_posts_response.assert_called_once_with(
            post_id_list)
